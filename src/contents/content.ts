...
const availableServices = servicesConfig.availableServices as Record<
  string,
  string[]
>
...
if (!(window as any)._formatScriptInitialized) {
  ...
  document.addEventListener("mouseup", (event) =>
    debouncedMouseSelection(event as MouseEvent)
  )
  ...
  if (typeof chrome !== "undefined" && chrome.storage?.onChanged) {
    chrome.storage.onChanged.addListener((changes, areaName) => {
      ...
      if (typeof change?.newValue === "boolean") {
        ...
        console.log(
          "[SOCx] Updated floatingButtonsEnabled to:",
  ...
  const clampToViewport = (
    ...
    const maxLeft = Math.max(
      BUTTON_MARGIN,
      docEl.clientWidth - width - BUTTON_MARGIN
    )
    const maxTop = Math.max(
      BUTTON_MARGIN,
      docEl.clientHeight - height - BUTTON_MARGIN
    )
    return {
  ...
  const getSelectionFromRootNode = (root: Node | null): Selection | null => {
    ...
    if (root instanceof ShadowRoot) {
      ...
      return typeof getSelectionFn === "function"
        ? getSelectionFn.call(root)
        : null
    }
    return null
  }
  ...
  function positionButtonGroup(geometry: SelectionGeometry | null) {
    ...
    const groupWidth =
      buttonGroup.offsetWidth || buttonGroup.getBoundingClientRect().width || 28
    const groupHeight =
    ...
    const verticalCenter =
      anchorRect.top + (anchorRect.height - groupHeight) / 2
    const horizontalCenter =
      anchorRect.left + (anchorRect.width - groupWidth) / 2

    const candidates = [
      { left: selectionRect.right + BUTTON_OFFSET, top: verticalCenter },
      {
        left: selectionRect.left - BUTTON_OFFSET - groupWidth,
        top: verticalCenter
      },
      { left: horizontalCenter, top: selectionRect.bottom + BUTTON_OFFSET },
      {
        left: horizontalCenter,
        top: selectionRect.top - BUTTON_OFFSET - groupHeight
      }
    ]
  ...
  const getSelectionGeometry = (
    ...
    if (
      ...
      const caretEstimate =
        estimateCaretRect(activeEl) ?? activeEl.getBoundingClientRect()
      caretRect = caretEstimate
      boundsRect = caretEstimate
      return { caret: caretRect, bounds: boundsRect }
    }
  ...
  const stripHiddenChars = (value: string): string =>
    value.replace(HIDDEN_CHAR_REGEX, "")
  ...
  async function handleSelection(event?: MouseEvent) {
    ...
    if (directType) {
      ioc = directType === "CVE" ? selectedText.toUpperCase() : selectedText
      type = directType
    } else {
    ...
    const nvdSupported = ["CVE"]
    const isSupported =
      vtSupported.includes(type) ||
      abuseSupported.includes(type) ||
      nvdSupported.includes(type)
    ...
    const button = isSupported
      ? createButton(ioc, async () => {
          try {
            ...
            await createTooltip(
              "❌ Error retrieving IOC information.",
              button,
              [],
              type
            )
    ...
    const magicButton = hasConfiguredServices
      ? createMagicButton(ioc, () => requestIOCInfo(ioc))
      : null
    if (magicButton) {
  ...
  function repositionButtons() {
    ...
    if (!hasUsableSelection(selection)) {
      selection = hasUsableSelection(lastValidSelection)
        ? lastValidSelection
        : null
    }
    const activeElement = getDeepActiveElement()
  ...
  function getIOCInfo(ioc: string): Promise<any> {
    ...
    const selectedServices = [
      type === "IP" ? "AbuseIPDB" : type === "CVE" ? "NVD" : "VirusTotal"
    ]
